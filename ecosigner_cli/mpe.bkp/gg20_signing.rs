...
async fn gg20_signing_original(args:Cli) -> Result<String> {
    ...
    let (my_index, incoming, outgoing) = join_computation_with_fixed_index(
        args.address.clone(),
        &format!("{}-offline", args.room),
        args.index,
    )
    .await
    .context("join offline computation (fixed index)")?;
    ...
    let (my_index_online, incoming, outgoing) = join_computation_with_fixed_index(
        args.address,
        &format!("{}-online", args.room),
        my_index,
    )
    .await
    .context("join online computation (fixed index)")?;

    if my_index_online != my_index {
        return Err(anyhow!(
            "online stage index mismatch: offline={}, online={}",
            my_index, my_index_online
        ));
    }
